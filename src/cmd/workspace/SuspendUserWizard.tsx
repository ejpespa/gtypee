...
import type { WorkspaceUserCommandDeps, SuspendUserResult } from './commands.js';
...
export function SuspendUserWizard({ userDeps, onCancel }: SuspendUserWizardProps) {
  ...
  const [result, setResult] = useState<SuspendUserResult | null>(null);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const res = await userDeps.suspendUser(email);
      if (res.applied) {
        setResult(res);
        setStep('DONE');
      } else {
        throw new Error('User suspension was not applied (failed).');
      }
    } catch (err: any) {
      setApiError(err.message || 'An error occurred while suspending the user.');
      setStep('CONFIRM');
    }
  };
  ...
  return (
    <Box flexDirection="column" padding={1} borderStyle="round" borderColor="blue">
      <Box marginBottom={1}>
        <Text bold color="cyan">Workspace Admin: Suspend User Wizard</Text>
      </Box>
      ...
      {step === 'CONFIRM' && (
        <Box flexDirection="column" marginTop={1}>
          ...
          <Box flexDirection="column">
            <Text bold color="yellow">Please confirm suspension of the user above.</Text>
            <Text color="white">Press <Text bold color="green">'y'</Text> to suspend the user, or <Text bold color="red">ESC</Text> to cancel.</Text>
          </Box>
        </Box>
      )}
      ...
      {step === 'SUBMITTING' && (
        <Box marginTop={1}>
          <Text color="yellow">Suspending user in Google Workspace...</Text>
        </Box>
      )}

      {step === 'DONE' && (
        <Box flexDirection="column" marginTop={1}>
          <Text color="green" bold>User suspended successfully!</Text>
          {result && (
            <Box flexDirection="column" marginTop={1}>
